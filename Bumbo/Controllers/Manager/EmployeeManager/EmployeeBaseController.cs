...
public abstract class EmployeeBaseController : NotificationController
{
    ...
    public EmployeeBaseController(UserManager<Employee> userManager, IEmployeeRepository employeeService,
        IMapper mapper, IBranchRepository branchService, IDepartmentsRepository departmentService)
    {
    ...
    public abstract IEnumerable<Employee> GetAllEmployees(int start = 0, int amount = int.MaxValue,
    ...
    public abstract int GetAmountOfEmployees(string searchString = "", bool includeActive = true,
        bool includeInactive = false);

    public IActionResult Index(string searchString, bool includeInactive = false, bool includeActive = true,
        EmployeeSortingOption currentSort = EmployeeSortingOption.Name_Asc, int page = 1)
    {
        if (page < 1) page = 1;
        ...
        var employees = GetAllEmployees((page - 1) * ItemsPerPage, ItemsPerPage, searchString, includeActive,
            includeInactive, currentSort);

        var resultingListViewModel = new EmployeeListIndexViewModel();
    ...
    {
        if (!ModelState.IsValid) return View("Views/EmployeeBase/ChangePassword.cshtml", viewModel);
    ...
    {
        if (!ModelState.IsValid) return View("Views/EmployeeBase/ToggleActive.cshtml", viewModel);
