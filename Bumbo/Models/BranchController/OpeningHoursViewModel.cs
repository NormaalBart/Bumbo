...
public class OpeningHoursViewModel : IValidatableObject
{
    ...
    public IEnumerable<ValidationResult> Validate(ValidationContext validationContext)
    {
        ...
        if (OpenTime == null)
            yield return new ValidationResult("Openingstijd mag niet leeg zijn als de winkel open is",
                new[] {"OpenTime"});

        if (CloseTime == null)
            yield return new ValidationResult("Sluitingstijd mag niet leeg zijn als de winkel open is",
                new[] {"CloseTime"});
