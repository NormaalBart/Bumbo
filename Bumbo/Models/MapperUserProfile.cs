    ...
    public MapperUserProfile()
    {
        ...
        CreateMap<Employee, EmployeeEditViewModel>()
            .ForMember(dest => dest.Birthdate,
                opt => opt.MapFrom(src => src.Birthdate.ToDateTime(new TimeOnly(0, 0, 0))))
        ...
        CreateMap<Branch, ListIndexBranchViewModel>()
            ...
            .ForMember(dest => dest.Managers,
                opt => opt.MapFrom(src =>
                    string.Join(", ", src.Managers.Select(model => model.FullName()).OrderBy(str => str).ToList())));
        ...
        CreateMap<BranchCreateViewModel, Branch>()
            ...
            .AfterMap((src, dest) =>
            {
                foreach (var standardOpeningHour in dest.StandardOpeningHours) standardOpeningHour.BranchId = dest.Id;
            });
        ...
        CreateMap<OpeningHoursViewModel, StandardOpeningHours>()
            .ForMember(dest => dest.OpenTime,
                opt => opt.MapFrom(src =>
                    src.OpenTime.HasValue ? new TimeOnly(src.OpenTime.Value.Ticks) : TimeOnly.MinValue))
            .ForMember(dest => dest.CloseTime,
                opt => opt.MapFrom(src =>
                    src.CloseTime.HasValue ? new TimeOnly(src.CloseTime.Value.Ticks) : TimeOnly.MinValue));
        CreateMap<StandardOpeningHours, OpeningHoursViewModel>()
            .ForMember(dest => dest.OpenTime,
                opt => opt.MapFrom(src =>
                    src.OpenTime.HasValue ? new TimeSpan(src.OpenTime.Value.Ticks) : TimeSpan.MinValue))
            .ForMember(dest => dest.CloseTime,
                opt => opt.MapFrom(src =>
                    src.CloseTime.HasValue ? new TimeSpan(src.CloseTime.Value.Ticks) : TimeSpan.MinValue));
        ...
        CreateMap<UnavailableMoment, UnavailableMomentsViewModel>().ForMember(dest => dest.EmployeeName,
