...
public class EmployeeRepository : Repository<Employee, string>, IEmployeeRepository
{
    ...
    public IEnumerable<Employee> GetAllManagers(int start = 0, int amount = int.MaxValue, string searchString = "",
    ...
    public int GetAmountOfManagers(string searchString = "", bool includeActive = true, bool includeInactive = false)
    {
        var users = Context.UserRoles.Where(role => role.RoleId == RoleType.MANAGER.RoleId)
            .Select(role => role.UserId).ToList();
        var employees = DbSet.Where(employee => users.Contains(employee.Id));
        return SortAndOrderBy(0, int.MaxValue, searchString, includeActive, includeInactive,
            EmployeeSortingOption.Name_Asc, employees).Count();
    }
    ...
    public IEnumerable<Employee> GetAllEmployeesOfBranch(int branch, int start = 0, int amount = int.MaxValue,
    ...
    {
        return DbSet.Include(e => e.PlannedShifts.Where(p => p.StartTime.Date == date.Date && p.BranchId == branchId))
            .Include(e =>
                e.WorkedShifts.Where(w => w.StartTime.Date == date.Date && w.EndTime != null && w.BranchId == branchId))
            .ToList().Where(e => e.PlannedShifts.Any() || e.WorkedShifts.Any());
    }
    ...
    private static IEnumerable<Employee> SortAndOrderBy(int start, int amount, string searchString, bool includeActive,
        bool includeInactive, EmployeeSortingOption sortingOption, IQueryable<Employee> employees)
    {
        if (!includeInactive && !includeActive)
            employees = employees.Where(e => e.Active);
        else if (!includeInactive && includeActive)
            employees = employees.Where(e => e.Active);
        else if (includeInactive && !includeActive) employees = employees.Where(e => !e.Active);
