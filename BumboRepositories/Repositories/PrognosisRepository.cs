...
public class PrognosisRepository : Repository<Prognosis>, IPrognosisRepository
{
    ...
    public IEnumerable<Prognosis> GetNextWeek(DateOnly firstDayOfWeek, int branchId)
    {
        ...
        var nextWeek = DbSet.Where(p =>
            p.Date >= firstDayOfWeek && p.Date <= firstDayOfWeek.AddDays(7) && p.BranchId == branchId);
        if (nextWeek.Count() == 7)
        ...
        for (var i = 0; i < 7; i++)
        {
            var temp = GetByDate(firstDayOfWeek.AddDays(i), branchId);
            if (temp == null)
            {
                var prognosis = new Prognosis();
                prognosis.Date = firstDayOfWeek.AddDays(i);
