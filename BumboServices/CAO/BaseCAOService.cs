...
public abstract class BaseCAOService: ICAOService
{
     ...
     */
    public Dictionary<ICAORule, List<PlannedShift>> VerifyPlannedShifts(List<PlannedShift> plannedShifts, DateOnly forDay)
    {
        if (plannedShifts.Count == 0)
        {
            return new Dictionary<ICAORule, List<PlannedShift>>();
        }
